...
## Sessão 2026-07-30 — Wave 2 Monumental (empresas maduras + advertoriais)

### Pipeline SEO — estado real (honesto)

**O que rodou:**
- DataForSEO keyword research (sessão anterior + esta sessão para Wave 2)
- gate-covers.mjs ✅ (0 violations em 13 posts)
- score-posts.mjs ✅ (composite=97, GEO=100, AEO=100 nos 10 posts Wave 1+ADV)

**O que NÃO rodou (e por que):**
- MiroFish ICP: persona automotiva — NÃO se aplica a blog contábil
- LLM Council (5 advisors): skill não executada nesta sessão
- EEAT check estruturado: verificação manual, não automatizada
- Image SEO check: cover images são placeholders (não existem fisicamente)

**Conclusão:** Pipeline "canônico" não foi completado. O que foi feito é válido para lançamento, mas LLM Council ficou pendente.

### ADV posts publicados (3 de 4 — ADV-01 removido)

| Slug | Composite | Status |
|---|---|---|
| `tributacao-dividendos-2026-lei-15270` | — | REMOVIDO (caso falso, não temos esse case) |
| `recuperacao-creditos-pis-cofins-2026` | 97 | ✅ live |
| `distribuicao-disfarcada-lucros-risco-fiscal` | 97 | ✅ live |
| `reforma-tributaria-impacto-empresas-brasilia-2026` | 97 | ✅ live |

**Lição ADV-01:** Nunca inventar case study. Se o post depende de "caso real da empresa X", ele só pode ir pro ar com o case confirmado pelo cliente.

### DataForSEO Wave 2 — volumes reais

| Keyword | Vol/mês | CPC | Decisão |
|---|---|---|---|
| split payment | 18.100 | R$0,30 | Adjacente — post de fluxo de caixa captura esse tráfego |
| juros sobre capital proprio | 720 | — | Alta intenção, escrever |
| transfer pricing | 1.000 | R$4,42 | CPC alto = intenção de contratar, escrever |
| lucro real lucro presumido | 70 | R$0,06 | Long-tail, escrever |
| stock options imposto de renda | 10 | — | Nicho puro, zero concorrência de conteúdo, escrever |
| IBS/CBS/lucros exterior/holding 2026 | N/A | — | Below threshold — B2B puro, GEO > volume |

### Wave 2 informational posts — em andamento (30/07)

8 posts escritos em paralelo:
1. `ibs-cbs-nota-fiscal-operacional-2026`
2. `lucro-real-quando-migrar-lucro-presumido`
3. `transfer-pricing-nova-metodologia-brasil`
4. `holding-familiar-receita-federal-2026` (ângulo: compliance 2026, diferente do holding existente)
5. `stock-options-tributacao-brasil-carf`
6. `juros-sobre-capital-proprio-lei-14789`
7. `split-payment-fluxo-caixa-empresa-2026`
8. `lucros-exterior-lei-14754-empresas-brasil`

### Deploy anterior (pré-Wave 2)

URL: https://385c7a38.blog-monumental.pages.dev (12 posts: 9 informativos + 3 ADVs)

---

## SESSÃO 2026-07-30 — Wave 2: 8 Posts Informativos Empresas Médias/Grandes

### Posts publicados nesta sessão (8 posts Wave 2)

| Slug | Palavras | Composite | Tema |
|---|---|---|---|
| `ibs-cbs-nota-fiscal-operacional-2026` | 3.970 | 97 | IBS/CBS NF-e campos agosto 2026 |
| `lucro-real-quando-migrar-lucro-presumido` | 3.972 | 97 | Decisão de regime tributário com exemplo numérico |
| `transfer-pricing-nova-metodologia-brasil` | 4.189 | 95 | IN 2.161/2023 obrigatória jan/2025 |
| `holding-familiar-receita-federal-2026` | 3.561 | 97 | Compliance/fiscalização (distinto do post patrimonial) |
| `stock-options-tributacao-brasil-carf` | 3.931 | 97 | CARF 2024: onerosidade=mercantil=ganho de capital |
| `juros-sobre-capital-proprio-lei-14789` | 2.187 | 100 | JCP pós-Lei 14.789: base congelada pré-2023 |
| `split-payment-fluxo-caixa-empresa-2026` | 3.231 | 97 | Float eliminado, cronograma, exemplo distribuidora |
| `lucros-exterior-lei-14754-empresas-brasil` | 3.977 | 97 | Lei 14.754/2023 tributação anual obrigatória jan/2024 |

### Correções aplicadas pós-score

- `split-payment`: agente esqueceu LeadForm e DimusHelp — adicionados manualmente (AEO 50→100)
- `juros-sobre-capital-proprio`: description 207 chars (>200=5pts) + title 71 chars (>70=12pts) — encurtados (SEO 77→100, composite 92→100)

**LIÇÃO**: ao usar agentes paralelos para escrever MDX, sempre rodar `score-posts.mjs` antes de assumir que os posts passam — agentes podem omitir componentes ou escrever description/title fora dos limites do scorer.

### Estado do blog após Wave 2

- **20 posts** totais no blog
- **17 passando** (composite ≥95): todos os 12 posts anteriores + 8 Wave 2 (menos os 3 pré-existentes legados)
- **3 falhando** (pré-existentes, não tocados): `como-trocar-de-contador-brasilia` (70), `fornecedores-governo-federal-brasilia-tributacao` (74), `reforma-tributaria-2026-ibs-cbs-split-payment-brasilia` (74)
- Deploy Wave 2: https://422c79a4.blog-monumental.pages.dev

### Pendências (não feitas nesta sessão)

- LLM Council (step 9 do pipeline canônico) para todos os posts — ainda pendente
- Imagens de cover (placeholders — todos os `.jpg` não existem fisicamente)
